...
class Scheduler:
    ...
    def sort_demand_by_user_edge_list(self, timeIndex: int, user_edge_list: list) -> list:
        ...
        for edge in user_edge_list:
            if self.demandPool.timeIndex_edge_left_map[timeIndex][edge] <= 0:
                continue
            demand = self.dataPool.edge_bandwidth_map[edge] - self.demandPool.timeIndex_edge_left_map[timeIndex][edge]
    ...
    # method two: sort edges by amount of remaining space
    # def sort_demand_by_user_edge_list(self, timeIndex: int, user_edge_list: list) -> list:
    #     sorted_user_edge_list = []
    #     for edge in user_edge_list:
    #         if self.demandPool.timeIndex_edge_left_map[timeIndex][edge] <= 0:
    #             continue
    #         demand = self.demandPool.timeIndex_edge_left_map[timeIndex][edge]
    #         sorted_user_edge_list.append((edge, demand))
    #     sorted_user_edge_list = sorted(sorted_user_edge_list, key=lambda tuple: tuple[1], reverse=True)
    #     return sorted_user_edge_list
        ...
        for timeIndex in list(self.demandPool.timeIndex_user_left_map.keys()):
            ...
            for user in self.demandPool.timeIndex_user_list_sorted_map[timeIndex]:
                ...
                while self.demandPool.timeIndex_user_left_map[timeIndex][user] > 0:
                    user_edge_list = self.dataPool.user_edge_list_map[user]
                    # TODO: edge with fewer stream at this time should provide more
                    temp = self.sort_demand_by_user_edge_list(timeIndex, user_edge_list)
                    print(temp)
                    edge = temp[0][0]
                    if self.demandPool.timeIndex_edge_left_map[timeIndex][edge] <= 0:
                        continue
                        ...
                        if self.demandPool.timeIndex_edge_left_map[timeIndex][edge] < \
                            ...
                            meetnum = int(self.demandPool.timeIndex_user_left_map[timeIndex][user] * 0.3)
                            if meetnum == 0:
                                meetnum = self.demandPool.timeIndex_user_left_map[timeIndex][user]
                            self.demandPool.timeIndex_edge_left_map[timeIndex][edge] -= meetnum
                            self.demandPool.timeIndex_user_left_map[timeIndex][user] = \
